...
flags.DEFINE_bool('add_dbof', True, "add Embedding to model")
flags.DEFINE_bool('add_netvlad', True, "add Embedding to model")
flags.DEFINE_bool('add_fisher_vector', True, "add Embedding to model")
flags.DEFINE_bool('add_moments', True, "add Embedding to model")
...
class EnsembleEarlyConcatAverageWithFCv3(models.BaseModel):
  """Creates a Deep Bag of Frames model.
  ...
  def create_model(self,
    ...
    video_activation = tf.concat([dbof_video, netvlad_video, fv_video], 1) 
...
class EnsembleEarlyConcatAverageWithFCv4(models.BaseModel):
  ...
  def create_model(self,
    ...
    def make_embedding(model_inputs, size, 
      ...
      return dbof, netvlad, fv, moments
    ...
    video_activation = tf.concat(embedding_video, 1) 
    video_activation = make_fc(video_activation, 'merge_video_embedding', fc_circulant, fc_add_batch_norm)

    audio_activation = tf.concat(embedding_audio, 1)
    audio_activation = make_fc(video_activation, 'merge_audio_embedding', fc_circulant, fc_add_batch_norm)
...
class EnsembleEarlyConcatAverageWithFCv7(models.BaseModel):
  """Creates a Deep Bag of Frames model.
  ...
  def create_model(self,
    ...
    def make_fc(input_, type_, name, circulant, batch_norm):
      if type_ == "video":
        ...
        size = fc_hidden_size // 2

      with tf.variable_scope(name):
        if not circulant:
          dim = input_.get_shape().as_list()[1] 
          fc_hidden_weights = tf.get_variable("{}_fc_hidden_weights".format(name),
            [dim, size], initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(size)))
          activation = tf.matmul(input_, fc_hidden_weights)
        else:
          input_dim = input_.get_shape().as_list()
          circ_layer_hidden = CirculantLayerWithFactor(input_dim, size, 
                      k_factor=k_factor, initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(size)))
          activation = circ_layer_hidden.matmul(input_)
      ...
      with tf.variable_scope(name):
        ...
        with tf.variable_scope("DBoF_max_{}".format(name), reuse=tf.AUTO_REUSE):
          ...
          if len(model_inputs) > 1:
            ...
            max_dbof = dbof_cls.forward(model_inputs[0])
          max_dbof = make_fc(max_dbof, name, 'max_dbof', fc_dbof_circulant, True)
        ...
        with tf.control_dependencies([max_dbof]):
          with tf.variable_scope("DBoF_avg_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              avg_dbof = avg_dbof_cls.forward(model_inputs[0])
            avg_dbof = make_fc(avg_dbof, name, 'avg_dbof', fc_dbof_circulant, True)
        ...
        with tf.control_dependencies([avg_dbof]):
          with tf.variable_scope("SoftDBoF_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              softdbof = softdbof_cls.forward(model_inputs[0])
            softdbof = make_fc(softdbof, name, 'softdbof', fc_dbof_circulant, True)
        ...
        with tf.control_dependencies([avg_dbof]):
          with tf.variable_scope("NetVLAD_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              netvlad = netvlad_cls.forward(model_inputs[0])
            netvlad = make_fc(netvlad, name, 'netvlad', fc_netvlad_circulant, True)
        ...
        with tf.control_dependencies([netvlad]):
          with tf.variable_scope("lightNetVLAD_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              light_netvlad = light_netvlad_cls.forward(model_inputs[0])
            light_netvlad = make_fc(light_netvlad, name, 'light_netvlad', fc_netvlad_circulant, True)
        ...
        with tf.control_dependencies([fv]):
          with tf.variable_scope("avg_var_pool_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              model_input = tf.reshape(model_inputs[0], [-1, max_frames, size])
              avg_pool, var_pool = tf.nn.moments(model_input, axes=1)
            avg_pool = make_fc(avg_pool, name, "avg_pool", fc_moment_circulant, True)
            var_pool = make_fc(var_pool, name, "var_pool", fc_moment_circulant, True)
        ...
        with tf.control_dependencies([avg_pool, var_pool]):
          with tf.variable_scope("max_pool_{}".format(name), reuse=tf.AUTO_REUSE):
            ...
            if len(model_inputs) > 1:
              ...
              model_input = tf.reshape(model_inputs[0], [-1, max_frames, size])
              max_pool = tf.reduce_max(model_input, 1)
            max_pool = make_fc(max_pool, name, "max_pool", fc_moment_circulant, True)

      return max_dbof, avg_dbof, netvlad, light_netvlad, fv, avg_pool, var_pool, max_pool
...
class EnsembleEarlyConcatAverageWithFCvF(models.BaseModel):
  """Creates a Deep Bag of Frames model.
